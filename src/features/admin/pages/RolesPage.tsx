...
import { PROTECTED_ADMIN_USER_ID, REVOCATION_CODE } from "@/lib/constants";
...
const RolesPage = () => {
  ...
  const handleRemove = (r: any) => {
    if (r.user_id === PROTECTED_ADMIN_USER_ID) {
      const code = window.prompt("⚠️ Este usuario es el super-administrador protegido.\nIngresa el código de seguridad para revocar su rol:");
      if (code === null) return;
      if (code.trim() !== REVOCATION_CODE) {
        toast.error("Código incorrecto. No se puede revocar este rol.");
        return;
      }
    }
    if (!window.confirm("¿Seguro que deseas remover este rol?")) return;
    removeRoleMutation.mutate({ id: r.id, userId: r.user_id });
  };
              ...
              <Card key={r.id} className="border-primary/10">
                <CardContent className="p-4 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {r.user_id === PROTECTED_ADMIN_USER_ID && (
                      <Badge variant="outline" className="text-[10px] bg-primary/10 text-primary border-primary/30">🔒 Protegido</Badge>
                    )}
                    <Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 h-8 w-8" onClick={() => handleRemove(r)}>
